...
class MoveParams(BaseModel):
    ...
    range: Range | None = Field(
        1,
        description="How close counts as arrived, in blocks. An ENUM by rule R4, and for a sharper reason than the other bounded fields: range was an unbounded number, and a move is satisfied the moment the body is ALREADY within range of the target, so a large value turns the verb into a guaranteed no-op that still reports success. Measured live 2026-07-31 under the unbounded schema, with the model emitting ranges of 100, 128 and 1000: across a 21-command sample the fleet travelled 6.4 blocks in TOTAL — about 0.3 blocks per move command (18 of 21 exactly zero, the rest under 3.5). Median command duration was 30ms across a larger 150-command window. Real walks do still happen when the model picks a target farther away than its own range — one 10.8-block journey was observed — so this is a heavy tax on movement rather than a total block, but at a third of a block per decision a villager cannot reach a treeline. The ledger records every one of these as 'arrived', so the villager never learns otherwise. Nothing above 8 has a legitimate caller; the far-target gate already rejects destinations past 128 blocks, so any range at that scale was a no-op by construction.",
    )
