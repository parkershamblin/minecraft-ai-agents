...
class MoveParams(BaseModel):
    ...
    range: Range | None = Field(
        1,
        description="How close counts as arrived, in blocks. An ENUM by rule R4, and for a sharper reason than the other bounded fields: range was an unbounded number, and a move is satisfied the moment the body is ALREADY within range of the target, so a large value turns the verb into a guaranteed no-op that still reports success. Measured live 2026-07-31 under the unbounded schema: 150 move commands completed in 25 minutes with a median duration of 30ms (136 of 150 under 100ms), and in a separate sample 10 of 11 completions reported blocksTraveled 0 — the rare non-zero walks were 0.8 to 2.8 blocks, a shuffle rather than a journey. The model was emitting ranges of 100, 128 and 1000. The ledger recorded every one of these as 'arrived', so the villager never learned otherwise. Nothing above 8 has a legitimate caller; the far-target gate already rejects destinations past 128 blocks, so any range at that scale was a no-op by construction.",
    )
