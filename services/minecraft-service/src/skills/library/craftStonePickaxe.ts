// Port of Voyager skill_library/trial1/skill/code/craftStonePickaxe.js
 ...
 */
export interface CraftStonePickaxePrimitives {
...
export type CraftStonePickaxeParams = Record<string, never>

const COBBLESTONE_NEEDED = 3
const STICKS_NEEDED = 2
const TABLE_SEARCH_DISTANCE = 32

export async function craftStonePickaxe(
  primitives: CraftStonePickaxePrimitives,
  _params: CraftStonePickaxeParams,
  ...
  // 4) Craft the pickaxe.
  const result = await primitives.craftItem({ name: 'stone_pickaxe', count: 1 })
 ...
 */
export const craftStonePickaxeSchema: SkillSchemaStub = {
  name: 'craftStonePickaxe',
  description:
    'Craft a stone pickaxe: mines missing cobblestone (3), crafts missing sticks (2), reuses or places a crafting table, then crafts.',
  params: {
    type: 'object',
    properties: {},
    required: [],
