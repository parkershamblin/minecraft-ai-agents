...
export class BotSession {
  ...
  // Same discipline as craft/gather: coded prescriptive failures (the message
  // is the villager's next percept), world-verified outcomes (the ghost-place
  // lesson: never trust a throw OR a silent success), honest partial results.

  private phaseACoded(code: string, message: string, retryable: boolean): Error {
  ...
  private requireStack(bot: Bot, item: string, verb: string) {
    const stack = bot.inventory.items().find((s) => s.name === item)
    if (!stack) {
      throw this.phaseACoded(
        'ITEM_NOT_CARRIED',
        `you carry no ${item.replace(/_/g, ' ')} to ${verb} — check your pack (gather or craft it first)`,
        false,
      )
    }
    return stack
  ...
  /** total carried count of an item — the executor's give-verification read */
  ...
  /**
   * Place ONE carried block. Explicit position must be within reach; omitted
   * position reuses the crafting-table spot scan beside the feet. The world
   * is read back after placement — a placeBlock throw with a landed block
   * still completes (the RB-1 placeCarried lesson, generalized).
   */
  async placeBlock(item: string, position?: Position): Promise<{ item: string; placed: string; position: Position }> {
    const bot = this.requireBot()
    const stack = this.requireStack(bot, item, 'place')
    const vecAt = (p: Position) => {
      const base = bot.entity.position.floored()
      return base.offset(p.x - base.x, p.y - base.y, p.z - base.z)
    }
    let spot: Position
    let groundPos: Position
    if (position) {
      const target = { x: Math.floor(position.x), y: Math.floor(position.y), z: Math.floor(position.z) }
      if (distance(this.position as Position, target) > 4.5) {
        throw this.phaseACoded(
          'PLACE_FAILED',
          `(${target.x}, ${target.y}, ${target.z}) is beyond arm's reach — placing is one block beside you, not construction; move there first`,
          false,
        )
      }
      const cell = bot.blockAt(vecAt(target))
      const below = bot.blockAt(vecAt({ ...target, y: target.y - 1 }))
      if (!cell || !(cell.name === 'air' || cell.name === 'cave_air')) {
        throw this.phaseACoded(
          'PLACE_FAILED',
          `the spot reads ${cell?.name ?? 'unloaded'}, not air — pick an empty cell`,
          false,
        )
      }
      if (!below || below.boundingBox !== 'block') {
        throw this.phaseACoded('PLACE_FAILED', 'nothing solid under that spot to set the block on', false)
      }
      spot = target
      groundPos = { ...target, y: target.y - 1 }
    } else {
      const found = pickTableSpot(this.position as Position, (p) => {
        const block = bot.blockAt(vecAt(p))
        return block
          ? {
              air: block.name === 'air' || block.name === 'cave_air',
      ...
      if (!found) {
        throw this.phaseACoded('PLACE_FAILED', 'no clear ground beside you — step somewhere more open and try again', true)
      }
      spot = found.spot
      groundPos = found.ground
    }
    await bot.equip(stack, 'hand')
    const ground = bot.blockAt(vecAt(groundPos))
    if (!ground) {
      throw this.phaseACoded('PLACE_FAILED', 'the ground under the spot is out of loaded range', true)
    }
    try {
      await bot.placeBlock(ground, ground.position.offset(0, 1, 0).minus(ground.position))
    } catch (err) {
      this.log.info({ err: err instanceof Error ? err.message : String(err), item }, 'placeBlock threw — verifying the world')
      await new Promise((resolve) => setTimeout(resolve, 1_000))
    }
    const placed = bot.blockAt(vecAt(spot))
    if (!placed || placed.name === 'air' || placed.name === 'cave_air') {
      throw this.phaseACoded(
        'PLACE_FAILED',
        `the ${item.replace(/_/g, ' ')} would not set (the spot still reads ${placed?.name ?? 'unloaded'}) — move to open ground and try again`,
        true,
      )
    }
    return { item, placed: placed.name, position: spot }
  }

  /**
   * Fill an empty bucket from the nearest source, or pour the carried
   * bucket's contents. The outcome reports what the world became — pouring
   * water over lava reads back obsidian/stone, which IS the percept the
   * casting drill needs.
   */
  async useBucket(
    mode: 'fill' | 'pour',
    ...
    if (mode === 'fill') {
      const empty = this.requireStack(bot, 'bucket', 'fill')
      const source = position
        ? bot.blockAt(vecAt({ x: Math.floor(position.x), y: Math.floor(position.y), z: Math.floor(position.z) }))
        : bot.findBlock({ matching: (b) => b.name === liquid, maxDistance: 4 })
      if (!source || source.name !== liquid) {
        throw this.phaseACoded(
          'LIQUID_NOT_FOUND',
          `no ${liquid} within reach to scoop — stand beside the ${liquid} (within ~4 blocks) and try again`,
          true,
        )
      }
      ...
      const filled = `${liquid}_bucket`
      if (this.countOf(filled) === 0) {
        throw this.phaseACoded(
          'LIQUID_NOT_FOUND',
          `the scoop came up empty (the ${liquid} may be flowing, not a source block) — aim at the still center of the ${liquid}`,
          true,
        )
      }
      const at = { x: source.position.x, y: source.position.y, z: source.position.z }
      return { mode, bucket: filled, targetBlock: bot.blockAt(source.position)?.name ?? 'unloaded', position: at }
    }
    // pour: whatever filled bucket is carried decides the liquid.
    const carried = bot.inventory.items().find((s) => s.name === 'water_bucket' || s.name === 'lava_bucket')
    if (!carried) {
      throw this.phaseACoded(
        'ITEM_NOT_CARRIED',
        'your buckets are all empty — fill one first (use_bucket mode "fill")',
        false,
      )
    }
    let target: Position
    if (position) {
      ...
      if (distance(this.position as Position, target) > 4.5) {
        throw this.phaseACoded('PLACE_FAILED', 'that spot is beyond arm\'s reach — walk closer before pouring', false)
      }
    } else {
      // The cell in front of the feet, by facing. Pouring blind is deliberate
      // — the caller who cares aims with `position`.
      const yaw = bot.entity.yaw
      const front = bot.entity.position.floored().offset(-Math.round(Math.sin(yaw)), 0, -Math.round(Math.cos(yaw)))
      target = { x: front.x, y: front.y, z: front.z }
    }
    await bot.equip(carried, 'hand')
    await bot.lookAt(vecAt(target).offset(0.5, 0.5, 0.5), true)
    bot.activateItem()
    await new Promise((resolve) => setTimeout(resolve, 1_000))
    if (this.countOf(carried.name) > 0 && this.countOf('bucket') === 0) {
      throw this.phaseACoded(
        'PLACE_FAILED',
        'the bucket never emptied — the pour target may be blocked; aim at an open cell or its floor',
        true,
      )
    }
    const after = bot.blockAt(vecAt(target))
    return { mode, bucket: carried.name, targetBlock: after?.name ?? 'unloaded', position: target }
  }

  /** Hold a carried item in the hand or off-hand (shields live off-hand). */
  async equipItem(item: string, destination: 'hand' | 'off_hand'): Promise<{ item: string; destination: string }> {
    const bot = this.requireBot()
    const stack = this.requireStack(bot, item, 'equip')
    await bot.equip(stack, destination === 'off_hand' ? 'off-hand' : 'hand')
    return { item, destination }
  }

  /**
   * Toss items on the ground, optionally aimed at a point first (the give
   * flow aims at the receiver so the drop lands on their side of the race —
   * the MINDcraft gift-ping-pong lesson).
   */
  async tossItem(item: string, count: number, toward?: Position): Promise<{ item: string; tossed: number }> {
    const bot = this.requireBot()
    const stack = this.requireStack(bot, item, 'toss')
    if (toward) {
      const base = bot.entity.position.floored()
      await bot.lookAt(base.offset(toward.x - base.x + 0.5, toward.y - base.y + 1.5, toward.z - base.z + 0.5), true)
    }
    const tossed = Math.min(count, this.countOf(item))
    await bot.toss(stack.type, null, tossed)
    return { item, tossed }
  }

  /** Deliberately eat/drink ONE named carried item (golden apple before a
   *  fight). Routine meals stay with the eat reflex. */
  async consumeItem(item: string): Promise<{ item: string; consumed: boolean }> {
    const bot = this.requireBot()
    const stack = this.requireStack(bot, item, 'consume')
    const before = this.countOf(item)
    await bot.equip(stack, 'hand')
    try {
      await bot.consume()
    } catch (err) {
      throw this.phaseACoded(
        'INVALID_PARAMS',
        `${item.replace(/_/g, ' ')} would not go down (${err instanceof Error ? err.message : String(err)}) — is it food?`,
        false,
      )
    }
    return { item, consumed: this.countOf(item) < before }
  }

  /** Chest I/O — the walk/search flow mirrors the crafting-table skill. */
  private async openNearestChest(): Promise<{ chest: Awaited<ReturnType<Bot['openContainer']>>; at: Position }> {
    const bot = this.requireBot()
    const found = bot.findBlock({
      matching: (candidate) => candidate.name === 'chest' || candidate.name === 'barrel',
      maxDistance: CRAFT_TABLE_SEARCH_DISTANCE,
    })
    if (!found) {
      throw this.phaseACoded(
        'CONTAINER_NOT_FOUND',
        `no chest stands within ${CRAFT_TABLE_SEARCH_DISTANCE} blocks — craft one (8 planks) and place_block it, or walk to the village chest first`,
        true,
      )
    }
    await bot.pathfinder.goto(new goals.GoalNear(found.position.x, found.position.y, found.position.z, 2))
    const chest = await bot.openContainer(found)
    return { chest, at: { x: found.position.x, y: found.position.y, z: found.position.z } }
  }

  async deposit(item: string, count: number): Promise<{ item: string; deposited: number; chestAt: Position }> {
    const bot = this.requireBot()
    this.requireStack(bot, item, 'deposit')
    const typeId = bot.registry.itemsByName[item]?.id
    if (typeId === undefined) {
      throw this.phaseACoded('INVALID_PARAMS', `'${item}' is not an item this world knows`, false)
    }
    const { chest, at } = await this.openNearestChest()
    try {
      const toPut = Math.min(count, this.countOf(item))
      await chest.deposit(typeId, null, toPut)
      return { item, deposited: toPut, chestAt: at }
    } finally {
      chest.close()
    }
  }

  async withdraw(item: string, count: number): Promise<{ item: string; withdrawn: number; chestAt: Position }> {
    const bot = this.requireBot()
    const typeId = bot.registry.itemsByName[item]?.id
    if (typeId === undefined) {
      throw this.phaseACoded('INVALID_PARAMS', `'${item}' is not an item this world knows`, false)
    ...
    try {
      ...
      if (held === 0) {
        throw this.phaseACoded(
          'RESOURCE_NOT_FOUND',
          `the chest at (${at.x}, ${at.y}, ${at.z}) holds no ${item.replace(/_/g, ' ')} — someone may have taken it; check another chest or gather it yourself`,
          true,
        )
      }
      const toTake = Math.min(count, held)
      await chest.withdraw(typeId, null, toTake)
      return { item, withdrawn: toTake, chestAt: at }
    } finally {
      chest.close()
    }
  }
