...
| **memory-service** | Python 3.12, FastAPI, pgvector | `memories` | Generative-agents memory stream: importance/sentiment scoring, recency × importance × relevance retrieval, reflections. *Staged extraction executed in Sprint 2: born as an in-process module of agent-service over its own `memory_db`, extracted to a standalone service with the identical contract* | Decide anything — it remembers, it doesn't think |
